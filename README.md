...
| 문서 | 내용 |
|---|---|
| `docs/handover.md` | **인수인계** — 발표자료 제작 · 다른 PC 에서 시연 준비 · 하면 안 되는 것. **처음 받았다면 여기부터** |
...
> **저장소를 막 클론했다면 둘 다 없다.** 원본 데이터·임베딩·모델 가중치는 저장소에 넣지 않는다(이유는 [개인정보·보안](#개인정보보안)). 1 → 2 → 3 → 4 를 차례로 하거나, 이미 돌아가는 PC 에서 통째로 복사해 오면 된다 — **`docs/handover.md` §3** 에 두 방법이 다 있다.
